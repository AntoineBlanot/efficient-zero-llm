...
quantization_config = BitsAndBytesConfig(
    load_in_8bit=True,
    llm_int8_skip_modules=["token_head"]
)
model = RobertaForTokenClassification.from_pretrained(model_args.pretrained_model_name_or_path, device_map='auto', quantization_config=quantization_config)
# Define LoRA Config
...
data_collator = RobertaTokenCollator(tokenizer)
...
label_list = [x for x in label2id.keys()]
assert len(label_list) == model.base_model.model.config.n_class
